...
instr 2
iWavSel = p4
gklfo	oscil3  2700, 0.05, iWavSel
endin
...
a4      oscil3  (kModEnv+gklfo)*0.03,ifreq+(gklfo*0.0001),ifilter
a3      oscil3  kAmpEnv,(ifreq+a4)*.9999,ifilter
a2      oscil3  (kModEnv+gklfo)*0.02,ifreq+(gklfo*0.0001),ifilter
a1      oscil3  kAmpEnv,ifreq+a2,ifilter

outs    (a1/2)+(a3/2) *  kpan, (a1/2)+(a3/2) * (1 - kpan)

galeft    =         galeft  +  (a1/2+a3/2)*kpan     * irevSend
garight   =         garight +  (a1/2+a3/2)*(1-kpan) * irevSend
...
def RndStart():
    global start
    global total
    start =  (random.gauss(total/2,total/3))
    if start>total :
...
def RndEnvelope():
	...
	RndStart()
	global duration
	duration = max(random.gammavariate(5,5 ),0.1)
	global at
	global attack
